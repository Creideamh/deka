$(document).ready(function () {
    ...
    $("#add_beneficiary_row").unbind('click').bind('click', function() {
        ...
        for(x = 0; x < count_table_tbody_tr; x++) {
            ...
            totalSubAmount = Number(totalSubAmount) + Number($("#beneficiary_benefit_"+count).val());
        } // /for  
        totalSubAmount = totalSubAmount.toFixed(2);
                
        if (row_id <= 12) {  // Ensure 12 rows
            ...
            if(totalSubAmount < 100){
                ...
                var html = 
                    '<tr id="brow_'+row_id+'">'+
                        '<td><input type="text" name="beneficiary_name[]" id="beneficiary_name_'+row_id+'" class="form-control"></td>'+'<td>'+
                            '<select name="beneficiary_gender[]" id="beneficiary_gender_'+row_id+'"class="form-control beneficiary_gender_'+row_id+' select2 select2bs5">'+
                            '<option value=""></option>'+
                            '<option value="M">Male</option>'+
                            '<option value="F">Female</option>'+
                            '</select>'+
                        '</td>'+
                        '<td>'+'<input type="text" class="form-control"  id="beneficiary_date_'+row_id+'" onchange="displayTrust(\''+row_id+'\')"  name="beneficiary_date[]">'+'</td>'+
                        '<td>'+
                            '<select name="beneficiary_relationship[]" id="beneficiary_relationship'+row_id+'" class="form-control beneficiarySelect'+row_id+' select2bs5">'+
                                '<option value=""></option>'+
                            ...
                            '</select>'+
                        '</td>'+    
                        '<td>'+'<input type="number" min="0.0" max="100.00" onchange="checkPercentage(\''+row_id+'\')"  step="any" class="form-control" id="beneficiary_benefit_'+row_id+'"  name="beneficiary_benefit[]">'+'</td>'+
                        '<td>'+'<input type="text" class="form-control" id="beneficiary_contact_'+row_id+'" name="beneficiary_contact[]">'+'</td>'+
                        '<td>'+'<button class="btn btn-danger" id="browButton_'+row_id+'" onclick="removeBrow(\''+row_id+'\')"><i class="ti-close"></i></button>'+'</td>'+
                    '</tr>';
                    ...
                    $("#beneficiary_name_"+row_id).on("change keyup keypress keydown paste click", function(){
                        $("#beneficiary_date"+row_id).attr("required",true)
                        $("#beneficiary_gender_"+row_id).attr("required",true)
                        $("#beneficiary_relationship"+row_id).attr("required",true)
                        $("#beneficiary_benefit_"+row_id).attr("required",true)
                        $("#beneficiary_contact_"+row_id).attr("required",true)
                    })
